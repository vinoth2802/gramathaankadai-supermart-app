...
export default function Login() {
  ...
  const handleSubmit = (e) => {
    ...
    if (form.username === 'admin' && form.password === '1234') {
      localStorage.setItem('user', JSON.stringify({ username: 'admin', role: 'owner', loginAt: Date.now() }));
      navigate('/dashboard');
    } else {
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-2.5 rounded-xl transition shadow-md shadow-amber-200 mt-2"
            >
              Sign In
            </button>
          </form>
          ...
          <p className="text-center text-xs text-slate-400 mt-5">
            Default: <span className="font-mono font-semibold text-slate-500">admin / 1234</span>
          </p>
        </div>
      </div>
    </div>
