...
import { SalaryRecordsAPI } from '../../api/salaryRecords.js';
...
function calcSalary(emp, attData, month) {
  if (emp.employeeType === 'regular') {
    const gross = [emp.basicSalary, emp.hra, emp.da, emp.ta, emp.medicalAllowance, emp.specialAllowance]
      .reduce((s, v) => s + Number(v || 0), 0);
    const deductions = [emp.pf, emp.esi, emp.provisionalTax, emp.tds, emp.loanRecovery]
      .reduce((s, v) => s + Number(v || 0), 0);
    return { gross, deductions, net: gross - deductions, payableDays: null };
  }
...
function PayslipModal({ employee, month, attData, salaryRecord, onClose, onProcess }) {
  ...
  const earnings = isRegular ? [
    { label: 'Basic Pay',                  value: Number(employee.basicSalary      || 0) },
    { label: 'House Rent Allowance (HRA)', value: Number(employee.hra              || 0) },
    { label: 'Dearness Allowance (DA)',    value: Number(employee.da               || 0) },
    { label: 'Travel Allowance (TA)',      value: Number(employee.ta               || 0) },
    { label: 'Medical Allowance',          value: Number(employee.medicalAllowance || 0) },
    { label: 'Special Allowance',          value: Number(employee.specialAllowance || 0) },
  ].filter(e => e.value > 0) : [];

  const deductions = isRegular ? [
    { label: 'Provident Fund (PF)',  value: Number(employee.pf             || 0) },
    { label: 'ESI',                  value: Number(employee.esi            || 0) },
    { label: 'Provisional Tax',      value: Number(employee.provisionalTax || 0) },
    { label: 'TDS (Income Tax)',     value: Number(employee.tds            || 0) },
    { label: 'Loan / Advance',       value: Number(employee.loanRecovery   || 0) },
  ].filter(d => d.value > 0) : [];
