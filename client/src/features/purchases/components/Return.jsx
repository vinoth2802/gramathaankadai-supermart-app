...
import { PurchaseReturnsAPI } from '@features/purchases/resources/purchases-service';
...
function AddDebitNoteModal({ onClose, onSaved }) {
  ...
  const updateRow = (key, field, val) => setRows(prev => prev.map(r => {
    ...
    const safeVal = field === 'qty' ? normalizeQty(val) : val;
    const updated = { ...r, [field]: safeVal };
                      ...
                      return (
                        <tr key={row._key} className={`hover:bg-slate-50 ${row._fromInvoiceIdx !== undefined ? 'bg-orange-50/40' : ''}`}>
                          ...
                          <td className="px-2 py-2">
                            <input type="number" min="0" value={row.qty} onChange={e => updateRow(row._key, 'qty', e.target.value)}
