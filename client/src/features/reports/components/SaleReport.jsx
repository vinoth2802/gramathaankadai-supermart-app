...
const DATE_OPTIONS = ['All Sale Invoices', 'This Month', 'Last Month', 'This Quarter', 'This Year', 'Custom'];
...
export default function SaleReport() {
  ...
  } else if (filter !== 'All Sale Invoices') {
  ...
  const { data: rawSales = [], isLoading } = useQuery({
    queryKey: ['report-sales', filter, customFrom, customTo],
  ...
  const rows = rawSales
    .map(s => ({
      ...
      date:        s.date,
      invoice:     s.invoice,
      party:       (s.party?.name ?? s.customerName ?? 'Walk-in Customer').toUpperCase(),
      amount:      Number(s.grandTotal ?? 0),
      received:    Number(s.totalReceived ?? 0),
      balance:     Math.max(0, Number(s.grandTotal ?? 0) - Number(s.totalReceived ?? 0)),
  ...
  const presetRange = filter !== 'All Sale Invoices' && filter !== 'Custom' ? getPresetRange(filter) : null;

  return (
    <div className="space-y-4">
      <h1 className="text-xl font-bold text-slate-800">Sale Report</h1>
      ...
      <div className="flex items-center gap-3 flex-wrap bg-white border border-slate-200 rounded-xl px-4 py-3 shadow-sm">
        ...
        <div ref={ref} className="relative">
          ...
          {open && (
            <div className="absolute top-full left-0 mt-1.5 bg-white border border-gray-200 rounded-lg shadow-lg min-w-[190px] z-50 overflow-hidden">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-emerald-50 border border-emerald-100 rounded-xl px-5 py-4 shadow-sm">
          ...
          <p className="text-2xl font-bold text-emerald-700">{fmtAmt(totalReceived)}</p>
        </div>
        <div className="bg-rose-50 border border-rose-100 rounded-xl px-5 py-4 shadow-sm">
          <p className="text-xs text-slate-500 mb-1">Total Balance Due</p>
          <p className="text-2xl font-bold text-rose-700">{fmtAmt(totalBalance)}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <h2 className="text-sm font-bold text-slate-700">Sale Invoices</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[800px]">
            <thead>
              <tr>
                {['Date', 'Invoice No', 'Party Name', 'Payment Mode', 'Amount', 'Received', 'Balance', 'Status'].map(h => (
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                ...
                <tr><td colSpan={8} className="text-center py-12 text-slate-400 text-sm">No sale invoices found for the selected period.</td></tr>
              ) : rows.map(row => (
                <tr key={row.id} className="hover:bg-slate-50/80 transition">
                  ...
                  <td className="px-4 py-3 text-xs text-emerald-700 text-right">{fmtAmt(row.received)}</td>
                  <td className="px-4 py-3 text-xs text-rose-600 text-right">{fmtAmt(row.balance)}</td>
